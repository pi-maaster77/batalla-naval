...
public class Barco extends JLabel {
    ...
    public Barco(Integer ancho, Integer alto, Color color, Integer x, Integer y) {
        ...
        setOpaque(true);
        setBackground(color);
        setBounds(x * MainScreen.CELL_SIZE, y * MainScreen.CELL_SIZE, this.ancho * MainScreen.CELL_SIZE, this.alto * MainScreen.CELL_SIZE);
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                if (!MainScreen.enableDrag) return;
            ...
            public void mouseReleased(MouseEvent e) {
                ...
                if (MainScreen.draggedElement != null) {
                    Point releasePoint = SwingUtilities.convertPoint(MainScreen.draggedElement, e.getPoint(), MainScreen.grillaPropia);
                    Integer col = Math.max(0, Math.min((releasePoint.x - initialClick.x + MainScreen.CELL_SIZE / 2) / MainScreen.CELL_SIZE, MainScreen.GRID_COLS - MainScreen.draggedElement.ancho));
                    Integer row = Math.max(0, Math.min((releasePoint.y - initialClick.y + MainScreen.CELL_SIZE / 2) / MainScreen.CELL_SIZE, MainScreen.GRID_ROWS - MainScreen.draggedElement.alto));
                    Rectangle newBounds = new Rectangle(col * MainScreen.CELL_SIZE, row * MainScreen.CELL_SIZE, MainScreen.draggedElement.ancho * MainScreen.CELL_SIZE, MainScreen.draggedElement.alto * MainScreen.CELL_SIZE);
                    ...
                    if (!checkCollision(newBounds)) {
                        ...
                        MainScreen.draggedElement.setBounds(initialPosition.x, initialPosition.y, MainScreen.draggedElement.ancho * MainScreen.CELL_SIZE, MainScreen.draggedElement.alto * MainScreen.CELL_SIZE);
                    }
        ...
        addMouseMotionListener(new MouseMotionAdapter() {
            ...
            public void mouseDragged(MouseEvent e) {
                ...
                if (MainScreen.draggedElement != null) {
                    ...
                    x = Math.max(0, Math.min(x, MainScreen.grillaPropia.getWidth() - MainScreen.draggedElement.ancho * MainScreen.CELL_SIZE));
                    y = Math.max(0, Math.min(y, MainScreen.grillaPropia.getHeight() - MainScreen.draggedElement.alto * MainScreen.CELL_SIZE));
    ...
    protected void rotateElement() {
        ...
        Rectangle newBounds = new Rectangle(getX(), getY(), newAncho * MainScreen.CELL_SIZE, newAlto * MainScreen.CELL_SIZE);
    ...
    private void obtenerCasillasOcupadas() {
        ...
        for (int i = 0; i < ancho; i++) {
            for (int j = 0; j < alto; j++) {
                casillasOcupadas.add(new Integer[]{location.x / MainScreen.CELL_SIZE + i, location.y / MainScreen.CELL_SIZE + j});
            }
        }
    }
}
